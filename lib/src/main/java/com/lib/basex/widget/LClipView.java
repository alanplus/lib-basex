...
public class LClipView extends RelativeLayout implements OnTouchListener {
    ...
    public LClipView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public LClipView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        ...
        mStart = new PointF();
        mMid = new PointF();
        LayoutInflater.from(getContext()).inflate(R.layout.l_clip_picture_layout, this);
    ...
    public void setImage(String imagePath, Rect rect, int clipWhat, int degree) {
        mSrcBitmap = LBitmapUtils.getBitmapFromPath(imagePath);
    ...
    private Bitmap getBitmapFromUri(Uri uri, Context context) {
        ...
        try {
            ...
            options.inSampleSize = LBitmapUtils.calculateInSampleSize(options, screenWidth, screenHeight);
    ...
    private String saveHeadBitmap(Bitmap bitmap) {
        ...
        String userId = "head" + LUtils.getRandomId() + ".png";
        if (bitmap != null) {
            bitmap = setHeadBitmap(bitmap);
            String name = LPathUtils.getTempDir() + "/" + userId;
            boolean save = LBitmapUtils.save(bitmap, name);
            if (save) {
                return name;
            }
        }
